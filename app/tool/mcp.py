    ...
    async def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if not text_contents:
                return ToolResult(output="No output returned.")

            # For database tools, return the raw JSON text to preserve structure
            # This allows agents to parse the JSON properly
            if self.original_name in ["execute_sql", "get_table_schema", "list_tables"]:
                # Return the first text content as is (should be JSON)
                return ToolResult(output=text_contents[0])
            else:
                # For other tools, join content as before
                content_str = ", ".join(text_contents)
...
class MCPClients(ToolCollection):
    ...
    async def connect_sse(self, server_url: str, server_id: str = "") -> None:
        ...
        # Use a shorter server identifier to avoid tool name truncation
        if not server_id:
            # Generate a short hash-based identifier for the server URL
            import hashlib

            server_id = hashlib.md5(server_url.encode()).hexdigest()[:8]
        else:
            server_id = server_id
    ...
    async def connect_stdio(
        ...
        # Use a shorter server identifier to avoid tool name truncation
        if not server_id:
            # Generate a short hash-based identifier for the command
            import hashlib

            command_key = f"{command}_{'_'.join(args)}" if args else command
            server_id = hashlib.md5(command_key.encode()).hexdigest()[:8]
        else:
            server_id = server_id
    ...
    def _sanitize_tool_name(self, name: str) -> str:
        ...
        # Truncate to 80 characters if needed (leave more room for tool names)
        if len(sanitized) > 80:
